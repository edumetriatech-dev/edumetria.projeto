...
const Dashboard = () => {
  ...
  const chartData = [
    ...
    { name: 'Médio Risco', value: stats.mediumRisk, color: 'hsl(38, 92%, 50%)' },
  ...
  return (
    <div className="p-6 md:p-8">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-muted-foreground">
            Análise de risco de evasão • Última atualização: {new Date().toLocaleDateString('pt-BR')}
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <KPICard icon={Users} title="Total de Alunos" value={stats.total} subtitle="analisados" variant="default" />
        <KPICard icon={AlertTriangle} title="Risco Alto" value={stats.highRisk} subtitle={`alunos (${stats.highRiskPercent}%)`} variant="high" />
        <KPICard icon={AlertCircle} title="Risco Médio" value={stats.mediumRisk} subtitle={`alunos (${stats.mediumRiskPercent}%)`} variant="medium" />
        <KPICard icon={CheckCircle} title="Risco Baixo" value={stats.lowRisk} subtitle={`alunos (${stats.lowRiskPercent}%)`} variant="low" />
      </div>

      {/* Charts and Priority Section */}
      <div className="grid lg:grid-cols-5 gap-6 mb-8">
        <div className="lg:col-span-2 bg-card rounded-xl border border-border shadow-sm p-6">
          <h2 className="text-lg font-semibold text-foreground mb-4">Distribuição de Risco</h2>
        ...
        <div className="lg:col-span-3 bg-card rounded-xl border border-border shadow-sm p-6">
          <h2 className="text-lg font-semibold text-foreground mb-4">Prioridades de Intervenção</h2>
          <div className="space-y-4">
            {topRiskStudents.map((student) => (
              <div key={student.id} className="flex items-center gap-4 p-4 bg-muted/50 rounded-lg hover:bg-muted transition-colors">
                <div className="flex-1">
                  ...
                  <ProgressBar value={student.probability} level={student.riskLevel} />
                </div>
      ...
      <div className="bg-card rounded-xl border border-border shadow-sm">
        <div className="p-6 border-b border-border">
          <h2 className="text-lg font-semibold text-foreground mb-4">Lista Completa de Alunos</h2>
